...
# 앞에서 여러 줄로 적은 값이나 설정의 묶음을 마무리함.
    ...
    """첫 값을 피벗으로 삼아 세 그룹으로 나누는 퀵 정렬이다.

    items는 입력 목록, key는 비교값 함수, reverse=True는 내림차순이다.
    입력 목록은 바꾸지 않고 새 목록을 반환한다.
    평균 O(N log N), 최악 O(N²)이며 키 비교 비용은 별도다.
    이 구현은 less·equal·greater 안의 입력 순서를 유지하여 안정적이다.
    일반적인 제자리 퀵 정렬까지 안정적이라는 뜻은 아니다.
    명시적인 작업 스택으로 재귀 깊이 제한은 피하지만 최악 시간은 남는다.
    """
    if len(items) <= 1:
