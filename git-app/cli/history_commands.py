...
class HistoryCommands:
    ...
    def handle_log(self, args: list[str], options: dict[str, str]) -> None:  # 기본 또는 기준별 로그를 선택함.
        if "sort-by" not in options:  # 정렬 옵션이 없으면 부모 우선 로그임.
            show_log(self._graph.get_topological_log())  # 브랜치 이름표와 함께 출력함.
            return  # 기본 로그 출력을 마침.
    ...
    def handle_path(self, args: list[str], options: dict[str, str]) -> None:  # 두 커밋 사이의 경로를 찾음.
